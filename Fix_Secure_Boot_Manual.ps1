<#
    ...
    7.	Starts the VM and do Manual Upgrade
    8.	Checks that the guest OS is fully online
    9.	Installs the new boot certificates
    10.	Shuts down the VM and clears Setup Mode
    11.	Boots the VM and sets AvailableUpdates to 0x5944 (certs ready for install)
    12.	Reboots until AvailableUpdates becomes 0x4100 (may require multiple reboots)
    13.	Reboots and runs Secure-Boot-Update again
...
$MaxTries       = 10
$DelaySeconds   = 10
$VMDKPath       = "[VMware_NFS_Lun01] Secureboot/secureboot.vmdk"
# =============================================================================
# FUNCTIONS
# =============================================================================
...
write-host " [Step 4 PK remediation] ..." -ForegroundColor Cyan
write-log " [Step 4 PK remediation] ..."

 # ------------------------------------------------------------------
            # Step 4 - PK remediation
            #
...
# Add Disk
# Add-VMDK -VMname $VM -VMDKPath $VMDKPath
Add-VMDK -VMName $VM -VMDKPath $VMDKPath

# https://knowledge.broadcom.com/external/article/423919
New-AdvancedSetting -Entity $vm -Name "uefi.allowAuthBypass" -Value "TRUE" -Force -Confirm:$false
...
Write-Host "EFI setup will be opened on next boot for $vm" -ForegroundColor Green
Write-Log "EFI setup will be opened on next boot for $vm"

Start-VM $VM | Out-Null

# ------------------------------------------------------------------
# Manual Replace Certs & Ask for Continu
# ------------------------------------------------------------------


Write-Host ""
Write-Host "WARNING: This will replace/enroll Secure Boot PK and append KEK 2023 on VM: $($VM.Name)" -ForegroundColor Yellow
Write-Host "Make sure you have a snapshot and BitLocker is handled/suspended if used." -ForegroundColor Yellow

Write-Log "WARNING: This will replace/enroll Secure Boot PK and append KEK 2023 on VM: $($VM.Name)"
Write-Log "Make sure you have a snapshot and BitLocker is handled/suspended if used."

$confirmCertReplace = Read-Host "Continue with Secure Boot certificate replacement? Type YES to continue"

if ($confirmCertReplace -ne "YES") {
    Write-Host "Certificate replacement cancelled for $($VM.Name)." -ForegroundColor Red
    Write-Log "Certificate replacement cancelled by user. Skipping VM."
    break
}
...
# Remove the Added 100MB disk
Get-HardDisk -VM $vm | Where-Object {$_.Name -ne "Hard disk 1" -and ($_.CapacityGB * 1024) -ge 95 -and ($_.CapacityGB * 1024) -le 105 } | Remove-HardDisk -Confirm:$false


write-host " [Step 5 PK Change] Download Cer Files..." -ForegroundColor Cyan
write-log " [Step 5 PK Change] Download Cer Files..."
...
write-host "Step 6 Install New Boot Certs" -ForegroundColor Green
write-log "Step 6 Install New Boot Certs"
